...
import { groupBy, parseFont, arrayPush, hex2rgb, replaceAll } from "./Utils";

let isDebug: boolean = false;

const doubleResizeMaxWidth: T_doubleResizeMaxWidth = {
    full: {
      legacy: 3020,
      default: 3550,
    },
...
class NiconiComments {
  ...
  private nicoScripts: {
    replace: any[];
    ban: any[];
    default: T_nicoScript_default[];
    reverse: any[];
  };
  private readonly collision_right: any;
  private readonly collision_left: any;
  private readonly collision_ue: any;
  private readonly collision_shita: any;
  ...
  preRendering(rawData: formattedComment[], drawAll: boolean) {
    ...
    let parsedData: parsedComment[] = this.getCommentPos(
                  ...
                  if (
                    ...
                    if (posY + comment.height > 1080) {
                      if (1080 < comment.height) {
                        ...
                        posY = Math.floor(
                          Math.random() * (1080 - comment.height)
                        );
                      }
                      is_break = true;
                      break;
   ...
   * @param preRendering
   */
  getTextImage(i: number, preRendering: boolean = false) {
   ...
   */
  parseCommand(comment: any): parseCommandResult {
    let metadata = comment.mail,
      loc: T_commentLoc | null = null,
      size: T_commentSize | null = null,
      fontSize_ = null,
      color = null,
      font: T_commentFont | null = null,
      full = false,
      ender = false,
      _live = false,
      invisible = false,
    ...
    for (let i in metadata) {
      ...
      if (size === null) {
        switch (command) {
          case "big":
            size = "big";
            fontSize_ = fontSize.big.default;
            break;
          case "small":
            size = "small";
            fontSize_ = fontSize.small.default;
            break;
        }
      }
      if (color === null) {
  ...
  parseCommandAndNicoscript(
    ...
    let color = "#FFFFFF",
      size: T_commentSize = "medium",
      font: T_commentFont = "defont",
      loc: T_commentLoc = "naka";
    for (let i in this.nicoScripts.default) {
      const value = this.nicoScripts.default[i];
      if (!value) continue;
      if (value.long !== null && value.start + value.long < comment.vpos) {
    ...
    for (let i in this.nicoScripts.replace) {
      if (
        this.nicoScripts.replace[i].long !== null &&
        this.nicoScripts.replace[i].start + this.nicoScripts.replace[i].long <
          comment.vpos
      ) {
        this.nicoScripts.default = this.nicoScripts.default.splice(
          Number(i),
    ...
    if (this.showCommentCount) {
      ...
      if (this.timeline[vpos]) {
        this.context.strokeText(
          "Count:" + this.timeline[vpos]!.length,
          100,
          200
        );
...
const changeCALayer = (rawData: formattedComment[]): formattedComment[] => {
  ...
  for (let i in rawData) {
    ...
    let key = `${value.content}@@${Array.from(
        new Set(JSON.parse(JSON.stringify(value.mail)).sort() as string[])
      )
  ...
  for (let i in data) {
    const value: formattedComment = data[i]!;
    if (
      userList[value.user_id] !== undefined &&
      userList[value.user_id]! >= 10
    ) {
